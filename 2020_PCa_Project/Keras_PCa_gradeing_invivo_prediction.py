...
class data(object):
    ...
    def __init__(
        ...
        self.pred_PCa_file      = pred_PCa_file
        self.pred_prostate_file = pred_PCa_file    
    ...
    def data_prediction(self):
        ...
        df_pred_PCa = pd.read_csv(os.path.join(pred_dir, self.pred_PCa_file))
        df_pred_prostate = pd.read_csv(os.path.join(pred_dir, self.pred_prostate_file))
        
        x_pred = (df_pred_PCa.iloc[:, 7:30])/(df_pred_prostate.iloc[:, 7:30].max())

        return df_pred_PCa, x_pred
...
def prediction():
    ...
    x_index = np.asarray(df_pred_PCa.iloc[:, [1]])[:,0]
    y_index = np.asarray(df_pred_PCa.iloc[:, [2]])[:,0]
    z_index = np.asarray(df_pred_PCa.iloc[:, [3]])[:,0]

    img = np.zeros(shape=(280, 224, 24))

    for i in range(x_index.shape[0]):
            img[x_index[i], y_index[i], z_index[i]] = y_prediction_label[i] + 1
    ...
    cf1 = len(ROI_img[ROI_img == 1])/ROI_img.size
    cf2 = len(ROI_img[ROI_img == 2])/ROI_img.size
    cf3 = len(ROI_img[ROI_img == 3])/ROI_img.size
    cf4 = len(ROI_img[ROI_img == 4])/ROI_img.size
    cf5 = len(ROI_img[ROI_img == 5])/ROI_img.size
    ...
    return prediction_map, cf1, cf2, cf3, cf4, cf5
...
if __name__ == '__main__':
    ...
    class3_ratio   = 1.0
    class4_ratio   = 1.0
    input_DBSI_DTI = np.arange(7,30)
    input_DBSI     = [1,13,14,18,19,20,22,26,27,28,29,31,32,33,35,36,37,38,39,40]
    input_DTI      = [2, 3, 8, 11]
    x_input        = input_DBSI_DTI
    ...
    prediction_map     = 'grading' + '_' + str(learning_rate) + '_' + str(batch_momentum) + '_' + str(dropout_rate) + '_' + '.nii'
    ...
    df_pred_PCa, x_pred = data.data_prediction() 
    ...
    prediction_map, cf1, cf2, cf3, cf4, cf5 = prediction()
    ...
    print("Results:")
    print("Grade 1 Confidence: " + str(cf1))
    print("Grade 2 Confidence: " + str(cf2))
    print("Grade 3 Confidence: " + str(cf3))
    print("Grade 4 Confidence: " + str(cf4))
    print("Grade 5 Confidence: " + str(cf5))
