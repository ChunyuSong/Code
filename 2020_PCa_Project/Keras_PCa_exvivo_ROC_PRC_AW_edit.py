...
class PCa_data(object):
    ...
    def data_construction_sample_split(self): #does train, test, validation split. First by sample, then by voxel
        ...
        #9:1 samplewise tts;
        num_test      = int(np.ceil((len(unique_samples)/10)))
...
if __name__ == '__main__':
    ...
    loop_times     = list(range(10))
    Learning_rate  = [0.1] #optimized
    Momentum       = [0.99] #optimized
    Dropout_rate   = [0]
    Batch_size     = [200]
    Epochs         = [20]
    N_neurons      = [100]
                    ...
                    for m in N_neurons:
                        ...
                        for n in Dropout_rate:
                            ...
                            for x_test, y_test in zip(x_test_list, y_test_list):
                                ...
                                print(cm)
                                ACC_, TPR_, TNR_, PPV_, NPV_, FPR_, FNR_, FDR_ = model_evaluation(cm, 3).statistics_2()
                                ACC_list.append(ACC_)
                                TPR_list.append(TPR_)
                                TNR_list.append(TNR_)
                                PPV_list.append(PPV_)
                                NPV_list.append(NPV_)
                                FPR_list.append(FPR_)
                                FNR_list.append(FNR_)
                                FDR_list.append(FDR_)
