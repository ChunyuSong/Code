...
def main(file_path = '',bin=128):
    ...
    for i in range(len(roi_name)):
        ...
        if all(value == 0 for value in idx):
            ...
            plt.rc('xtick', labelsize=30, direction='out')
            plt.rc('ytick', labelsize=30, direction='out')
            ...
            ax.savefig(os.path.join(pathname,'Plot_scatter_%s.png'%(roi_name[i])),format='png')
            plt.close()

            ## DBSI and DIT histogram plot
            plt.switch_backend('TkAgg')
            plt.rc('font', weight='bold')
            plt.rc('font', style='normal')
            plt.rc('xtick', labelsize=50, direction='in')
            plt.rc('ytick', labelsize=50, direction='in')
            plt.rcParams['axes.linewidth'] = 5
            ...
            ax.bar(bin_edges[0:len(bin_means)], bin_means, bin_width, align='center', color=color, edgecolor='black', linewidth=2)
            ax.set_ylim(0, 0.6)
            ...
            #ax.set_xlabel('Isotropic ADC',fontweight='bold',fontsize=30)
            #ax.set_ylabel('Fraction Bin Means',fontweight='bold',fontsize=30)
            ...
            #dti_handle = [DTI_ADC]
            ax_dti.bar(bin_edges[0:len(bin_means)], bin_count, bin_width, align='center', color='blue', edgecolor='black', linewidth=2)
            ax_dti.set_xlim(np.log10(min_adc),np.log10(max_adc))
            ax_dti.set_xticklabels(ticks)
            #ax_dti.set_xlabel('ADC',fontweight='bold',fontsize=30)
            #ax_dti.set_ylabel('Count',fontweight='bold',fontsize=30)
            #ax_dti.legend(handles=dti_handle, loc=1)
            ax_dti.spines['top'].set_visible(False)
            ax_dti.spines['right'].set_visible(False)
            plt.savefig(os.path.join(pathname,'DTI_%s.png'%(roi_name[i])),format='png')
            plt.close()
