...
def main(file_path = '',bin=128):
    ...
    for i in range(len(roi_name)):
        ...
        if all(value == 0 for value in idx):
            ...
            #plt.rc('xtick', labelsize=45, direction='out')
            #plt.rc('ytick', labelsize=45, direction='out')
            ...
            plt.rc('xtick', labelsize=60, direction='out')
            plt.rc('ytick', labelsize=60, direction='out')
            fig = plt.figure(figsize=(20,20))
            ax_dhi = fig.add_subplot(2,1,1)
            ...
            #RF1 = mpatches.Patch(color=color[0], label='Highly-Restricted')
            #RF2 = mpatches.Patch(color=color[1], label='Restricted')
            #HF = mpatches.Patch(color=color[2], label='Hindered')
            #WF = mpatches.Patch(color=color[3], label='Free')
            #handles=[RF1,RF2,HF,WF]
            #handles = np.asarray(handles)
            color = np.asarray(color)
            color = np.repeat(color,new_idx,axis=0)
            ax_dhi.bar(bin_edges[0:len(bin_means)],bin_means,bin_width,align='edge',color=color)
            ax_dhi.set_ylim(0,0.7)
            #ticks = np.arange(0, 0.7, 0.2)
            ax_dhi.set_yticks(ticks_dhi)
            ax_dhi.set_xlim(np.log10(min_adc),np.log10(max_adc+min_adc))
            ax_dhi.set_xticks([])
            #ax_dhi.grid(color='grey', linestyle='-', linewidth=3)
            #ax_dhi.spines['right'].set_visible(False)
            #ax_dhi.spines['top'].set_visible(False)
            #ax_dhi.set_yticks([])
            fig.canvas.draw()
            ...
            #ax.set_xlabel('Isotropic ADC',fontweight='bold',fontsize=50)
            #ax.set_ylabel('Fraction',fontweight='bold', fontsize=50)
            #ax.legend(handles=handles[non_zero_idx].tolist(), loc=2)
            ...
            ax_dti.set_ylim(0,15)
            ticks_dti = np.arange(0, 15, 3)
            ax_dti.set_yticks(ticks_dti)
            ax_dti.set_xlim(np.log10(min_adc),np.log10(max_adc))
            ax_dti.set_xticks([])
            #ax_dti.spines['right'].set_visible(False)
            #ax_dti.spines['top'].set_visible(False)
            #ax_dti.grid(color='grey', linestyle='-', linewidth=3)
            #ax_dti.set_yticks([])
            #ax_dti.yaxis.set_ticks_position('left')
            #ax_dti.xaxis.set_ticks_position('bottom')
            #ax_dti.set_xticklabels(ticks)
            #ax_dti.set_xlabel('ADC',fontweight='bold', fontsize=50)
            #ax_dti.set_ylabel('Count',fontweight='bold', fontsize=50)
            #ax_dti.legend(handles=dti_handle, loc=2)
