...
######################### qc check #####################################################################################
                ...
                for img_idx in range(co.shape[1]):
                    imdx = list(co[:,img_idx])
                    img_dti_adc[imdx[0],imdx[1],imdx[2]] = adc_data['DTI_ADC'][0,img_idx]
               # for img_idx in range(co.shape[1]):
                   # imdx = list(co[:,img_idx])
                   # img_dti_fa[imdx[0],imdx[1],imdx[2]] = adc_data['DTI_FA'][0,img_idx]
                fig = plt.figure(figsize=(24,15))
                # ax_1 = fig.add_subplot(2,1,1)
                ax_1 = fig.add_subplot(1,1,1)
                ax_1.axis('off')
                ax_1.imshow(img_dti_adc[:,:,idx[2,1]],cmap='gray',vmin=0.2,vmax=2.0,aspect='equal')
                ax_1.imshow(temp_atlas[:,:,idx[2,1]],cmap='rainbow',alpha=0.35,aspect='equal')
                ax_1.set_title('DTI ADC',fontweight='bold',fontsize=40)
...
########################################################################################################################                
                ...
                for img_idx in range(co.shape[1]):
                    imdx = list(co[:,img_idx])
                    img_fr[imdx[0],imdx[1],imdx[2]] = adc_data['Fractions'][:,img_idx]
