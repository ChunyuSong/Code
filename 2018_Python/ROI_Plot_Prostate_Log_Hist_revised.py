...
def main(file_path = '',bin=128):
    ...
    for i in range(len(roi_name)):
        ...
        if all(value == 0 for value in idx):
            ...
            new_idx = [len(bin_edges[bin_edges<np.log10(rf1_max + adc_constant)]),len(bin_edges[bin_edges<np.log10(rf2_max + adc_constant)]) - len(bin_edges[bin_edges<np.log10(rf1_max + adc_constant)]),len(bin_edges[bin_edges<np.log10(hfr_max + adc_constant)]) - len(bin_edges[bin_edges<np.log10(rf2_max + adc_constant)]),len(bin_edges[bin_edges<np.log10(wfr_max + adc_constant)]) - len(bin_edges[bin_edges<np.log10(hfr_max + adc_constant)])]
            ...
            ax.bar(bin_edges[0:len(bin_count)], bin_count/len(adc_nonneg), bin_width, align='center', color=color, edgecolor='black', linewidth=2)
            # ax.set_ylim(0,1)
            ...
            plt.savefig(os.path.join(pathname,'Plot_log_hist_freq_%s.png'%(roi_name[i])),format='png')
            plt.close()

            fig = plt.figure(figsize=(24,15))
            ax = fig.add_subplot(1,1,1)
            ax.bar(bin_edges[0:len(bin_count)], bin_means, bin_width, align='center', color=color, edgecolor='black', linewidth=2)
            ax.set_xlim(np.log10(min_adc),np.log10(max_adc+min_adc))
            ax.set_xticklabels(ticks)
            ax.set_xlabel('ADC',fontweight='bold')
            ax.set_ylabel('Fraction Bin Means',fontweight='bold')
            ax.legend(handles=handles[non_zero_idx].tolist(), loc=1)
            plt.savefig(os.path.join(pathname,'Plot_log_hist_mean_%s.png'%(roi_name[i])),format='png')
            plt.close()


            fig = plt.figure(figsize=(24,15))
            ax = fig.add_subplot(1,1,1)
            ax.bar(bin_edges[0:len(bin_count)], bin_means*(bin_count/len(adc_nonneg)), bin_width, align='center', color=color, edgecolor='black', linewidth=2)
            ax.set_xlim(np.log10(min_adc),np.log10(max_adc+min_adc))
            ax.set_xticklabels(ticks)
            ax.set_xlabel('ADC',fontweight='bold')
            ax.set_ylabel('Fraction Bin Means*Frequency',fontweight='bold')
            ax.legend(handles=handles[non_zero_idx].tolist(), loc=1)
            plt.savefig(os.path.join(pathname,'Plot_log_hist_mean_freq_%s.png'%(roi_name[i])),format='png')
            plt.close()
            ...
            dti_handle = [DTI_ADC]
            ax_dti.bar(bin_edges[0:len(bin_count)], dti_bin_count/len(adc_nonneg), bin_width, align='center', color='blue', edgecolor='black', linewidth=2)
            ax_dti.set_xlim(np.log10(min_adc),np.log10(max_adc))
            ax_dti.set_xticklabels(ticks)
            ax_dti.set_xlabel('ADC',fontweight='bold')
            ax_dti.set_ylabel('Frequency',fontweight='bold')
            ax_dti.legend(handles=dti_handle, loc=1)
            plt.savefig(os.path.join(pathname,'Plot_log_hist_dti_freq_%s.png'%(roi_name[i])),format='png')
            plt.close()
