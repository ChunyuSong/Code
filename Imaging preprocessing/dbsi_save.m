...
if nargin < 1
    ...
    prompt = {'Cell Threshold(range from 0 to ):','Hinderded Water Threshold(Range from):','Hinderded Water Threshold(Range to):','CSF Threshold(Range to infinity):'};
    ...
    if isempty(iso_threshold)
        prompt = {'Cell Threshold(range from 0 to ):','Hinderded Water Threshold(Range from):','Hinderded Water Threshold Threshold(Range to):','CSF Threshold(Range to infinity):'};
        dlg_title = 'Input thresholds to differentiate isotropic components';
        num_lines = 1;
        def = {'0.3','0.3','3.0','3.0'};
        answer = inputdlg(prompt,dlg_title,num_lines,def);
...
d3rd_isotropic_grid = cRawData.DBSI_iIsoSpecGrid > iso_threshold(4);
...
restricted_ratio_map = zeros(map_dimensions);
restricted_ratio_map(roi_index) =  d1st_isotropic_ratio;
restricted_adc_map = zeros(map_dimensions);
restricted_adc_map(roi_index) =  d1st_isotropic_diff;
hindered_ratio_map = zeros(map_dimensions);
hindered_ratio_map(roi_index) =  d2nd_isotropic_ratio;
hindered_adc_map = zeros(map_dimensions);
hindered_adc_map(roi_index) =  d2nd_isotropic_diff;   
water_ratio_map = zeros(map_dimensions);
water_ratio_map(roi_index) =  d3rd_isotropic_ratio;
water_adc_map = zeros(map_dimensions);
water_adc_map(roi_index) =  d3rd_isotropic_diff;
